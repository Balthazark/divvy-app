...
export default function BalanceModal() {
  ...
  const totalAverageDebt =
    purchases.docs.reduce((a, p) => a + p.data().price, 0) / -group.users.length;
  const usersBalance = group.users.reduce(
    (a: any, v: any) => ({ ...a, [v]: totalAverageDebt }),
    {}
  );
  purchases.forEach((d) => usersBalance[d.data().createdBy] = usersBalance[d.data().createdBy] + d.data().price )
...
function UserCard(props:{userId:string,debt:number}){
   ...
   return(
    ...
    <Text className="font-medium text-base">{user.name} {user.lastName}</Text>
    <Text className="font-medium text-base" style={{color:debtStyle}}>{props.debt.toFixed(2)} kr</Text>
</View>
   )
